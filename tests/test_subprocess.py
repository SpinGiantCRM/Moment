...
class TestSandboxPreexec:
    def test_setsid_called(self) -> None:
        ...
        with (
            patch("moment.utils.subprocess.os.setsid") as mock_setsid,
            patch("moment.utils.subprocess.os.getuid", return_value=1000),
            patch("moment.utils.subprocess.os.path.isdir", return_value=False),
    ...
    def test_setsid_graceful_failure(self) -> None:
        ...
        with (
            patch("moment.utils.subprocess.os.setsid", side_effect=OSError),
            patch("moment.utils.subprocess.os.getuid", return_value=1000),
            patch("moment.utils.subprocess.os.path.isdir", return_value=False),
            patch("moment.utils.subprocess.os.closerange"),
        ):
...
class TestRunSandboxed:
    def test_adds_preexec_fn(self) -> None:
        ...
        with (
            patch("moment.utils.subprocess._try_qprocess", return_value=None),
    ...
    def test_preserves_existing_preexec_fn(self) -> None:
        ...
        mock_result = subprocess.CompletedProcess(args=["echo"], returncode=0, stdout="", stderr="")
