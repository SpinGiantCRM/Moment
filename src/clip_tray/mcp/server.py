...
tools from :mod:`clip_tray.mcp.tools`.  Serves as the glue between
...
def create_server(allow_mutations: bool = False) -> "FastMCP":
    ...
    if not _FASTMCP_AVAILABLE:
        print(
            ...
            "    pip install clip-tray[mcp]\n"
            "or  pip install fastmcp",
            file=sys.stderr,
        )
        raise ImportError("fastmcp is required for MCP server")

    from clip_tray.mcp.tools import register_all_tools

    server = FastMCP(name="clip-tray", description="Clip management pipeline MCP server")
