"""CLI entry point for the ``clip-tray bot`` subcommand.

Parses arguments, checks the optional dependency, wires up a
:class:`~clip_tray.core.discord_bot.DiscordBot` instance, and blocks
until the bot exits.
"""
...
def run_bot(argv: list[str] | None = None) -> int:
    ...
    from clip_tray.core.discord_bot import _DISCORD_AVAILABLE as _available

    if not _available:
        print(
            "discord.py not installed.  Run:\n"
            "    pip install clip-tray[bot]\n"
