...
class Store:
    ...
    # -- Clips
    def insert_clip(self, clip): return self.clips.insert(clip)
    def get_clip(self, clip_id): return self.clips.get(clip_id)
    def update_clip(self, clip): return self.clips.update(clip)
    def delete_clip(self, clip_id, soft=True): return self.clips.delete(clip_id, soft=soft)
    def list_clips(self, **kwargs): return self.clips.list(**kwargs)
    def count_clips(self, **kwargs): return self.clips.count(**kwargs)

    # -- Tags
    def set_tags(self, clip_id, tags): self.tags.sync_for_clip(clip_id, tags)
    def list_tags(self, limit: int = 100, offset: int = 0): return self.tags.get_all(limit, offset)
    def delete_tag(self, tag_id): self.tags.delete(tag_id)

    # -- Edit profiles
    def get_edit_profile(self, clip_id): return self.profiles.get_edit_profile(clip_id)
    def save_edit_profile(self, profile): return self.profiles.save_edit_profile(profile)

    # -- Bookmarks
    def insert_bookmark(self, bm): return self.bookmarks.insert(bm)
    def get_bookmarks_for_session(self, session_stem):
        return self.bookmarks.get_for_session(session_stem)
    def delete_bookmark(self, bookmark_id): self.bookmarks.delete(bookmark_id)
    ...
    def save_webhook(self, wh):
        ...
        raw = Webhook(id=wh.id, url=encrypted, name=wh.name, enabled=wh.enabled,
                      notify_on=wh.notify_on, per_game_filter=wh.per_game_filter,
    ...
    def list_webhooks(self):
        ...
        for wh in rows:
            ...
            if m:
                ...
                redacted = real_url
            result.append(Webhook(id=wh.id, url=redacted, name=wh.name,
                                  enabled=wh.enabled, notify_on=wh.notify_on,
    ...
    def delete_webhook(self, webhook_id): self.webhooks.delete(webhook_id)
    def insert_webhook_log(self, entry): return self.webhooks.insert_log(entry)

    # -- Folders
    def save_folder(self, folder): return self.folders.save(folder)
    def list_folders(self, limit: int = 100, offset: int = 0):
        return self.folders.list_all(limit, offset)
    def delete_folder(self, folder_id): self.folders.delete(folder_id)

    # -- Game profiles
    def save_game_profile(self, profile): return self.profiles.save_game_profile(profile)
    def get_game_profile(self, game_name): return self.profiles.get_game_profile(game_name)
    def list_game_profiles(self, limit: int = 100, offset: int = 0):
        return self.profiles.list_game_profiles(limit, offset)
    def delete_game_profile(self, game_name): self.profiles.delete_game_profile(game_name)

    # -- Stats & logs
    def get_aggregate_stats(self): return self.clips.get_aggregate_stats()
    def list_webhook_logs(self, **kwargs): return self.webhooks.list_logs(**kwargs)
    def clear_webhook_logs(self): self.webhooks.clear_logs()
    ...
    # -- Trash
    def restore_clip(self, clip_id): return self.clips.restore(clip_id)
    def empty_trash(self): return self.clips.empty_trash()

    # -- URL history
    def insert_url_history(self, clip_id, url): self.clips.insert_url_history(clip_id, url)
    def get_url_history(self, clip_id): return self.clips.get_url_history(clip_id)

    # -- Tasks
    def insert_task(self, task): return self.tasks.insert(task)
    def get_pending_tasks(self, limit=10): return self.tasks.get_pending(limit)
    ...
    def has_active_task_for_clip(self, clip_id): return self.clips.has_active_task_for_clip(clip_id)
    def batch_soft_delete_clips(self, clip_ids): return self.clips.batch_soft_delete(clip_ids)
    ...
    def _migrate_discord_user_id(self): pass
    def _migrate_webhook_include_url(self): pass
